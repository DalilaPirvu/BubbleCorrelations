...
program Gross_Pitaevskii_1d
  ...
  integer :: nTime = 64
  integer :: sim, nSims = 10000, minSim = 9000
  integer :: spec = nyq/16
...
!!!!!!!!!! 
  ...
  subroutine output_fields(fld, dt, dtout, sim)
    ...
    inquire(file='/gpfs/dpirvu/bubble_correlations/free_corr_x'//trim(str(nLat))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lambda))//'_spec'//trim(str(spec))//'_sim'//trim(str(sim))//'_fields.dat', opened=o)
    if (.not.o) then
       open(unit=oFile,file='/gpfs/dpirvu/bubble_correlations/free_corr_x'//trim(str(nLat))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lambda))//'_spec'//trim(str(spec))//'_sim'//trim(str(sim))//'_fields.dat')
