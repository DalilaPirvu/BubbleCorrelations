...
program Gross_Pitaevskii_1d
  ...
  integer :: nTime = 512
  integer :: sim, nSims = 100, minSim = 10 
  integer :: spec = 256, n_cross = 2
...
!!!!!!!!!! 
  ...
  subroutine output_fields(fld, dt, dtout, sim)
    ...
    inquire(file='/gpfs/dpirvu/bubble_correlations/kcut256_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_len'//trim(real_str(len))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lambda))//'_alph'//trim(real_str(alph))//'_spec'//trim(str(spec))//'_sim'//trim(str(sim))//'_fields.dat', opened=o)
    if (.not.o) then
       open(unit=oFile,file='/gpfs/dpirvu/bubble_correlations/kcut256_t'//trim(str(nTime))//'_x'//trim(str(nLat))//'_len'//trim(real_str(len))//'_phi0'//trim(real_str(phi0))//'_lamb'//trim(real_str(lambda))//'_alph'//trim(real_str(alph))//'_spec'//trim(str(spec))//'_sim'//trim(str(sim))//'_fields.dat')
